...
OKTETO_IMAGE ?= okteto/styled-javascript-patterns
DOCKER_IMAGE ?= alexanderr/styled-javascript-patterns
TAG ?= latest
...
.PHONY: helm-start
helm-start:
	helm upgrade --install backend-javascript-patterns -f charts/values.yaml --create-namespace --namespace webapp charts
